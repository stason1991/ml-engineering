...
async def setup_demo_data(session: AsyncSession):
    ...
    # 2. Проверяем и создаём демо-пользователя (ТЕПЕРЬ ВНУТРИ setup_demo_data)
    res = await session.execute(select(User).where(User.login == "user_demo"))
    demo_user = res.scalars().first()

    if not demo_user:
        # Создаём пользователя
        demo_user = User(login="user_demo", password_hash="demo_hash_999")
        ...
        # Создаём запись в истории (исправлено под структуру твоей модели)
        ...
        print(f"Демо-user '{demo_user.login}' и все связанные данные созданы.")
