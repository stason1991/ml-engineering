    ...
    try:
        ...
        try:
            params = pika.ConnectionParameters(
                ...
                heartbeat=600,
                connection_attempts=3, # 3 попытки
                retry_delay=2          # через каждые 2 сек
            )
            connection = pika.BlockingConnection(params)
            connection.close()
            logger.info("RabbitMQ connection successful")
        except Exception as rabbit_err:
            # Если RabbitMQ не отвечает, не останавливаем приложение
            logger.warning(f"RabbitMQ is not ready yet: {rabbit_err}. Continuing startup...")

        logger.info("Application startup completed successfully")
        
    except Exception as e:
        # Критические ошибки по-прежнему останавливают запуск
